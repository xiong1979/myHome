define(function(require) {
	...
	var SqliteUtil = require("$UI/iHome/sqliteUtil");
...
//		this.database = window.openDatabase("X5_DEMO_DB", null, null, -1);
	...
	DemoDB.prototype.initData = function(onSuccess) {
		...
		// 检查表是否已存在
		var sqlCheckExists = "SELECT count(*) AS cnt FROM sqlite_master WHERE type = 'table' AND UPPER(name) in ('iHomeConfig') ";
		SqliteUtil.executeSql(db, sqlCheckExists, null, function(res) {
			if (res.rows.item(0).cnt < 2) {
				// 如果不存在则获取初始化脚本
				require(["text!$UI/iHome/demoDB.sql"], function(data) {
					executeSqlFile(data);
				});
			} else {
				doSuccess();
